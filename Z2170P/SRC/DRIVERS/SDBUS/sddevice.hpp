...
// default powerup wait while polling for NOT busy
    ...
    #define DEFAULT_POWER_UP_TOTAL_WAIT_TIME   2000*10
    #define EMMC_HIGHSPEED_MAXCLOCKRATE	       52000000 //52MHz
	#define EMMC_SWITCH2_1BITSBUS_ARGUMENT     0x03B70000
    #define EMMC_SWITCH2_4BITSBUS_ARGUMENT     0x03B70100
    #define EMMC_SWITCH2_8BITSBUS_ARGUMENT     0x03B70200
    #define EMMC_SWITCH2_HIGHSPEED_ARGUMENT    0x03B90100
	#define EMMC_INAND_32GB_SECTOR_NUMBER      0x03B70000
    #define EMMC_INAND_16GB_SECTOR_NUMBER      0x01DAA000
	#define EMMC_INAND_16GB_SECTOR_NUMBER_INT  0x01DACC00
    #define EMMC_INAND_8GB_SECTOR_NUMBER       0x00ECE000
    #define EMMC_INAND_4GB_SECTOR_NUMBER       0x00762C00
    #define EMMC_INAND_2GB_SECTOR_NUMBER       0x003AF000
    #define EMMC_INAND_1GB_SECTOR_NUMBER       0x001D7400
    #define EMMC_CSD_SPEC_VERSION_VALUE        0x10      
	
	const char cInandProductName04G[]         = {'G','4','0','M','E','S'};
	const char cInandProductName08G[]         = {'G','8','0','M','E','S'};
	const char cInandProductName16G[]         = {'G','6','1','M','E','S'};
	const char cInandProductName32G[]         = {'G','2','3','M','E','S'};
	const char cInandProductName16G_Builtin[] = {'G','6','1','M','T','S'};
#else
    #define DEFAULT_POWER_UP_TOTAL_WAIT_TIME   2000
#endif

#if defined(BSP_SDHIGHSPEEDSUPPORT_SDHC1) || defined(BSP_SDHIGHSPEEDSUPPORT_SDHC2)
    #define SDHC_HIGHSPEED_MAXCLOCKRATE	       50000000 //50MHz
#endif
...
#ifdef BSP_EMMCFEATURE
    DWORD EmulateSectorNumberByProdutID();
#endif
...
protected:
	...
	SD_API_STATUS GetCardBusWidthCapability(DWORD * pInterfaceMode);
	SD_API_STATUS TestCardBusWidth(SD_CARD_INTERFACE * psdCardInterface, BYTE * backupBuff);
protected:
    // BusRequest Queue
