...
// Need to fix this is memory leak...
    ...
    *pdwBytesTransferred = 0;
    ...
    case CSPROPERTY_VIDEOCONTROL_MODE:
        ...
        case CSPROPERTY_TYPE_SET:
            ...
            //but this video driver don't support STILL PIN, so we just return it           
    ...
    )
{
    ...
    case IOCTL_POWER_SET:
        ...
        if ( pOutBuf && OutBufLen >= sizeof(CEDEVICE_POWER_STATE) )
        {
            ...
            CEDEVICE_POWER_STATE CurrentPowerState = m_PowerState;

            // driver only supports D0 (on) and D4 (off), D0/1/2 = ON, D3/4 = OFF
            m_PowerState = *pState;
            if (CurrentPowerState <= D2 && *pState >= D3)
			    PowerDown();
			else if (CurrentPowerState >= D3 && *pState <= D2)
			    PowerUp();
    ...
    case IOCTL_CONTEXT_RESTORE:
        DEBUGMSG(ZONE_IOCTL, (TEXT("CAM: IOCTL_CONTEXT_RESTORE\r\n")));
