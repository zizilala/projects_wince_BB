...
,   m_lpBusName(NULL)
{
    DEBUGMSG(ZONE_ENUM| ZONE_INIT,(TEXT("OmapBus::OmapBus (ActivateRegPath=%s)\r\n"),lpActiveRegPath));
...
BOOL OmapBus::Init()
{
    ...
    if ( m_lpActiveRegPath==NULL) {
        DEBUGMSG(ZONE_ENUM|ZONE_ERROR,(TEXT("REGENUM!Init RegOpenKeyEx() return FALSE!!!\r\n")));
        return FALSE;
    }
...
BOOL OmapBus::ActivateAllChildDrivers()
{
    ...
    while (pCurDevice) {
        ...
        for (DWORD dwIndex=0; dwIndex< m_dwNumOfInitParam; dwIndex++) {
            ...
            if (m_DeviceKey.IsKeyOpened() && 
                ...
                DEBUGMSG(ZONE_ENUM,(TEXT("ActivateChild add %s to %s ActivePath\r\n"),m_lpInitParamArray[dwIndex],pCurDevice->GetRegPath()));                
            }
        }
    ...
    while (dwCurOrder != MAXDWORD) {
        DEBUGMSG(ZONE_ENUM,(TEXT(" OmapBus::ActivateChild load device driver at order %d \r\n"),dwCurOrder));
...
DllEntry(HINSTANCE DllInstance, INT Reason, LPVOID Reserved)
{
    ...
    case DLL_PROCESS_ATTACH:
        ...
        DEBUGMSG(ZONE_INIT,(TEXT("OmapBus.DLL DLL_PROCESS_ATTACH\r\n")));
        DisableThreadLibraryCalls((HMODULE) DllInstance);
        break;
    case DLL_PROCESS_DETACH:
        DEBUGMSG(ZONE_INIT,(TEXT("OmapBus.DLL DLL_PROCESS_DETACH\r\n")));
        break;
    };
    return TRUE;
}
...
{
    DEBUGMSG(ZONE_INIT,(TEXT("OmapBus.DLL : +PowerUp dwData=%x \r\n"),dwData));
    DefaultBusDriver * pBusDriver = (DefaultBusDriver * )dwData;
    if (pBusDriver)
        pBusDriver->PowerUp();
    DEBUGMSG(ZONE_INIT,(TEXT("OmapBus.DLL : -PowerUp dwData=%x \r\n")));
...
{
    DEBUGMSG(ZONE_INIT,(TEXT("OmapBus.DLL : +PowerDown dwData=%x \r\n"),dwData));
    DefaultBusDriver * pBusDriver = (DefaultBusDriver * )dwData;
    if (pBusDriver)
