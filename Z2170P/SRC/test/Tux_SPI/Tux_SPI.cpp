...
//
//  Module: Tux_PowerManagementTest.cpp
...
DWORD       g_BufferSize = 512;
DWORD       g_NbThreads = 30;
DWORD	      g_Writes = 50;
DWORD	      g_ChipSelect = 0;
DWORD	      g_Port = 1;
...
{
    static  TCHAR   szHeader[] = TEXT("Tux_SPI: ");
    TCHAR   szBuffer[1024];
...
static BOOL  SetTestOptions( LPTSTR lpszCmdLine )
{
    ...
    //TCHAR*	pTmp		= NULL;
	DWORD   dwIndex     = 0;
    char    szTempString[11]; 
    PDWORD pdwVariable = NULL;
    ...
    while( '\0' != lpszCmdLine[iCmdIdx] )
    {
        ...
        case CmdThread:
            {
                g_pKato->Log( LOG_DETAIL, TEXT("Set Number of threads \n"));
                pdwVariable = &g_NbThreads;
                break;
            }
        case CmdBuffer:
            {
                g_pKato->Log( LOG_DETAIL, TEXT("Set size of buffer \n"));
                pdwVariable = &g_BufferSize;
                break;
            }
        case CmdWrites:
            {
                g_pKato->Log( LOG_DETAIL, TEXT("Set Number of writes \n"));
                pdwVariable = &g_Writes;
                break;
            }
        case CmdChipSelect:
            {
                g_pKato->Log( LOG_DETAIL, TEXT("Set Chip select \n"));
                pdwVariable = &g_ChipSelect;
                break;
            }
        case CmdPort:
            {
                g_pKato->Log( LOG_DETAIL, TEXT("Set Port \n"));
                pdwVariable = &g_Port;
                break;
            }
        default:
        ...
        //g_fStress = TRUE;
        /*g_pKato->Log( LOG_DETAIL, TEXT("In %s @ line %d: Option set number of iterations"),
            __THIS_FILE__, __LINE__ );*/
				...
				while(lpszCmdLine[iCmdIdx] != CmdSpace && lpszCmdLine[iCmdIdx] != '\0')
				{
                   ...
                   if(lpszCmdLine[iCmdIdx] >= '0' && lpszCmdLine[iCmdIdx]<='9')
				   {
					   //If the Lenght is greater than 10 digits return an error
                        if(10 == dwIndex)
						{
							g_pKato->Log( LOG_DETAIL, TEXT("In %s @ line %d:Value Not Valid "),__THIS_FILE__, __LINE__ );
							return FALSE;
						}
						szTempString[dwIndex] = (char)lpszCmdLine[iCmdIdx];
				   ...
				   {
					   	g_pKato->Log( LOG_DETAIL, TEXT("Invalid number \n"));
						return FALSE;
				   }

				}

			    szTempString[dwIndex] = '\0';
     			*pdwVariable = atoi(szTempString);
				//g_pKato->Log( LOG_DETAIL,TEXT("Number of Iterations Specified is %d"),g_StressCount);
