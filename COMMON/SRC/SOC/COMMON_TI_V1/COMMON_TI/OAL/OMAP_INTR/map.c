...
//
static UINT32 s_oalSysIntr2Irq[SYSINTR_MAXIMUM][OMAP_IRQ_PER_SYSINTR];
static UINT32 s_oalIrq2SysIntr[OMAP_IRQ_MAXIMUM];
    ...
    )
{
    ...
    for (i = 0; i < OMAP_IRQ_PER_SYSINTR; i++)
        ...
        if (s_oalIrq2SysIntr[irq] == sysIntr)
            {
            s_oalIrq2SysIntr[irq] = (DWORD) SYSINTR_UNDEFINED;
            }
        }
