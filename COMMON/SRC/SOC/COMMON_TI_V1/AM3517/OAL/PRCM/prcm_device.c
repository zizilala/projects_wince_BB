    ...
    )
{
    ...
    switch (devId)
        {
        case OMAP_DEVICE_OHCI:    // 48MHz FCLK
        case OMAP_DEVICE_EHCI:    // 120MHz FCLK
        ...
        case OMAP_DEVICE_USBHOST3:
            ...
            PrcmDeviceEnableFClock(OMAP_DEVICE_OHCI, bEnable);
            ...
            if (pEntry->pSrcClocks->size == 4 &&
                ...
                PrcmDeviceEnableFClock(OMAP_DEVICE_EHCI, bEnable);
                }
            break;
    ...
    )
{
    ...
    switch (devId)
        ...
        case OMAP_DEVICE_USBHOST2:
        case OMAP_DEVICE_USBHOST3:
            PrcmDeviceEnableIClock(OMAP_DEVICE_OHCI, bEnable);
            break;
        ...
        case OMAP_DEVICE_USBHOST3:
            ...
            if (rgClocks[0] == kUSBHOST_120M_FCLK)
                ...
                if (pEntry->pfclk->refCount > 0)
                    {
                    PrcmDeviceEnableFClock(OMAP_DEVICE_EHCI, TRUE);
                    }
                }
            else
                {
                ...
                if (pEntry->pfclk->refCount > 0)
                    {
                    PrcmDeviceEnableFClock(OMAP_DEVICE_EHCI, FALSE);
                    }
                }
            break;
    ...
    )
{
    ...
    case OMAP_DEVICE_VPFE:
        {
            ...
            // WARNING : no ref counter here! (it's not shared so ref count is not useful
            if (bEnable)
            {
				dwReg = INREG32(&g_pSysCtrlGenReg->CONTROL_IPSS_CLK_CTRL);
                dwReg |= (VPFE_VBUSP_CLK_EN | VPFE_FUNC_CLK_EN);
				OUTREG32(&g_pSysCtrlGenReg->CONTROL_IPSS_CLK_CTRL, dwReg);
				dwReg = INREG32(&g_pSysCtrlGenReg->CONTROL_IPSS_CLK_CTRL);
            }
            else
            {
    ...
    case OMAP_DEVICE_HECC:
        {
            //EnterCriticalSection(&g_IPSS_cs);
            // WARNING : no ref counter here! (it's not shared so ref count is not useful
            if (bEnable)
            {
                g_pSysCtrlGenReg->CONTROL_IPSS_CLK_CTRL |= HECC_VBUSP_CLK_EN;
            }
            else
            {
                g_pSysCtrlGenReg->CONTROL_IPSS_CLK_CTRL &= ~HECC_VBUSP_CLK_EN;
            }
            //LeaveCriticalSection(&g_IPSS_cs);
	...
	case OMAP_DEVICE_HSOTGUSB:
		{
            ...
            //EnterCriticalSection(&g_IPSS_cs);
            // WARNING : no ref counter here! (it's not shared so ref count is useless)
            if (bEnable)
            {
				// Enable IPSS interface clock
				EnableDeviceClocks(OMAP_DEVICE_IPSS, TRUE);

				dwReg = INREG32(&g_pSysCtrlGenReg->CONTROL_IPSS_CLK_CTRL);
                dwReg |= (USB20OTG_VBUSP_CLK_EN | USB20OTG_FUNC_CLK_EN);
				OUTREG32(&g_pSysCtrlGenReg->CONTROL_IPSS_CLK_CTRL, dwReg);
				dwReg = INREG32(&g_pSysCtrlGenReg->CONTROL_IPSS_CLK_CTRL);
            ...
            else
            {
				...
				// Disable IPSS interface clock
				EnableDeviceClocks(OMAP_DEVICE_IPSS, FALSE);
            }
            //LeaveCriticalSection(&g_IPSS_cs);
	...
	case OMAP_DEVICE_CPGMAC:
		{
            ...
            if (bEnable)
            {
				...
				dwReg = INREG32(&g_pSysCtrlGenReg->CONTROL_IPSS_CLK_CTRL);
                dwReg |= (CPGMAC_FUNC_CLK_EN | CPGMAC_VBUSP_CLK_EN);
				OUTREG32(&g_pSysCtrlGenReg->CONTROL_IPSS_CLK_CTRL, dwReg);
				dwReg = INREG32(&g_pSysCtrlGenReg->CONTROL_IPSS_CLK_CTRL);
...
DeviceInitialize()
{
    ...
    for (i = 0; i < OMAP_DEVICE_COUNT - 1; ++i)
        {
        OALMSG(OAL_FUNC, (L"+DeviceInitialize: %d\r\n", i));
