...
;  File:  cpu.s
        ...
        INCLUDE kxarm.h
        INCLUDE armmacros.s
        INCLUDE omap3530_const.inc

        EXPORT OALCPUStart
        EXPORT OALCPUIdle
        EXPORT OALCPUEnd
        EXPORT OALUpdateCoreFreq
        EXPORT OALCPUWarmReset
...
;
 LEAF_ENTRY OALCPUIdle
        ...
        bleq       SaveContext
                                                 ...
                                                 ;--------------------------------------
        ldr        r1, [r4, #SDRC_REGS_OFFSET]                
        ldr        r5, [r1, #SDRC_POWER_OA]
    	orr        r5, r5, #0x40            ;enable self refresh on idle req
    	str        r5, [r1, #SDRC_POWER_OA] ;write back to SDRC_POWER register
...
|$_wait_sdrc_ok|
        ...
        ldr     r5, [r1, #SDRC_POWER_OA]
        bic     r5, r5, #0x40
        str     r5, [r1, #SDRC_POWER_OA]
                                            ;--------------------------------------
                                            ; Is dll in lock mode?
|$_wait_dll_lock|                                            
        ldr     r5, [r1, #SDRC_DLLA_CTRL_OA]        
        tst     r5, #0x4
        bxne    lr
                                            ;--------------------------------------
                                            ; wait till dll locks
        ldr     r5, [r1, #SDRC_DLLA_STATUS_OA]    
        and     r5, r5, #0x4
        cmp     r5, #0x4
        bne     |$_wait_dll_lock|
...
;------------------------------------------------------------------------------
;
;  Function:  OALCPUWarmReset
;
;  Perform a warm reset of the CPU and follow advisory 3.1.1.176 that requires
;  disabling the DDR before resetting for proper recovery
;
;  r0 = addr CPUIDLE_INFO
;
 LEAF_ENTRY OALCPUWarmReset
        ...
        mcr         p15, 0, r3, c7, c10, 5
                                                ;------------------------------
                                         ; setup pointers
                                         ; r5 = ref GLOBAL_PRM_REGS
                                         ; r6 = ref CORE_CM_REGS
        ldr         r5, [r0, #GLOBAL_PRM_REGS_OFFSET]
        ldr         r6, [r0, #CORE_CM_REGS_OFFSET]
                                         ;-------------------------------------
                                         ; Disable SDRC ICLK
        mov         r0, #SDRC_DISABLE_ICLK
        ...
        mov                    r3, #PRM_RSTCTRL_RST_GS_BIT
        str                    r3, [r5, #PRM_RSTCTRL_GLOBAL_OA]
                                         ;-------------------------------------
                                         ; Wait for ever until reset
LoopForEver
               bl                      LoopForEver

 ENTRY_END OALCPUWarmReset

;------------------------------------------------------------------------------

        ; ensure any literals used by above code are located before OALCPUEnd so they get copied to SRAM
...
;
 ...
 EXPORT OALGetL2Aux
 LEAF_ENTRY OALGetL2Aux
        mrc     p15, 1, r0, c9, c0, 2
        mov     pc, lr
 ENTRY_END OALGetL2Aux
